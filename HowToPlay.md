...
![alt text](<Screenshot 2026-08-22 220309.png>)
...
- `PLANT` — Plant a seed purchased from the market.
  ...
  - If you try to plant too many in a specific turn, none are planted — i.e. if you have 1 melon seed, but two units do the `PLANT MELON` command.
...
- `FERTILIZE` — Fertilize a plant to increase its potential yield (see harvest yields below). Doubles the per-day yield bonus for the next 3 days. The bonus only applies on days the plant is also watered (basic needs first).
...
- `BUY_LAND` — unlock a new 5x5 segment of land to plant on. Increasing in cost. Costs are: $1k, $2k, $4k.
...
- **One-time crops (wheat, carrot, melon):** Starting at half the plant's `max_yield_day` (Time to Max Yield) rounded up, watering during the bonus window will add one unit per day to the total harvestable yield. Fertilized plants add 2 per day instead.
...
![alt text](<Screenshot 2026-08-22 220355.png>)
...
![alt text](<Screenshot 2026-08-22 220508.png>)
![alt text](<Screenshot 2026-08-22 220522.png>)
...
6. **Day refresh** — if applicable, update the condition of plants and animals for a new day, and reset their fed/watered to condition to false.
7. **Market refresh** — modify the price of items on the market based on sells from previous turn.
8. **Income update** — update the player's bank based on any buys or sells.
9. **Farm update** — clear plants that have been harvested, items from the inventory that have been used or sold, add new plants/animals to the farm, etc.
